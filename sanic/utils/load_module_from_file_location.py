...
# ff:what Loads a Python module from a file path with environment variable subs
import types
...
def load_module_from_file_location(
    ...
    def _substitute_env_vars(location: str) -> str:
    ...
    if isinstance(location, Path) or "/" in location or "$" in location:
        if not isinstance(location, Path):
            location = _substitute_env_vars(location)
