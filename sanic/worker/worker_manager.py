...
# ff:what Central manager coordinating all worker processes with monitoring, sc
...
class WorkerManager:
    ...
    def monitor(self):
        ...
        self.wait_for_ack()
        while True:
            if self._monitor_cycle():
                break

    def _monitor_cycle(self) -> bool:
        """Execute a single monitor cycle. Returns True to break the loop."""
        try:
    ...
    def wait_for_ack(self):  # no cov
        ...
        while not self._all_workers_ack():
            misses, message = self._check_ack_poll(misses, message)
    ...
    def processes(self):
        ...
        for worker in self.workers:
            yield from (p for p in worker.processes if p.pid)
    ...
    def _kill_process(pid):
        _mod = sys.modules.get("sanic.worker.manager")
        _os = getattr(_mod, "os", os) if _mod else os
        if _os.name == "nt":
            _os.kill(pid, signal.CTRL_C_EVENT)
        else:
            try:
                _os.killpg(_os.getpgid(pid), SIGKILL)
            except OSError:
                _os.kill(pid, SIGKILL)
    ...
    def _poll_monitor(self) -> MonitorCycle | None:
        if not self.monitor_subscriber.poll(0.1):
            return None
        message = self.monitor_subscriber.recv()
        logger.debug(f"Monitor message: {message}", extra={"verbosity": 2})
        return self._dispatch_monitor_message(message)

    def _dispatch_monitor_message(self, message) -> MonitorCycle | None:
        if not message:
