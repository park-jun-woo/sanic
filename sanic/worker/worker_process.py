# ff:type feature=worker type=runner
# ff:what Individual worker process with lifecycle management, restart, and sta
...
class WorkerProcess:
    ...
    def start(self):
        _mod = sys.modules.get("sanic.worker.process")
        _os = getattr(_mod, "os", os) if _mod else os
        _os.environ["SANIC_WORKER_NAME"] = self.name
        _os.environ["SANIC_WORKER_IDENTIFIER"] = self.ident
        logger.debug(
        ...
        del _os.environ["SANIC_WORKER_NAME"]
        del _os.environ["SANIC_WORKER_IDENTIFIER"]
