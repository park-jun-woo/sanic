...
# ff:what Unix daemonization helper with double-fork, PID file, lock file, and
...
class Daemon:
    ...
    def __init__(
        ...
        _daemon_mod = sys.modules.get("sanic.worker.daemon")
        _os_is_win = (
            getattr(_daemon_mod, "OS_IS_WINDOWS", OS_IS_WINDOWS)
            if _daemon_mod
            else OS_IS_WINDOWS
        )
        if _os_is_win:
    ...
    def _validate_runtime_state(self) -> None:
        ...
        _daemon_mod = sys.modules.get("sanic.worker.daemon")
        _proc_exists = (
            getattr(_daemon_mod, "_process_exists", _process_exists)
            if _daemon_mod
            else _process_exists
        )
        _is_sanic = (
            getattr(_daemon_mod, "_is_sanic_process", _is_sanic_process)
            if _daemon_mod
            else _is_sanic_process
        )
        if _proc_exists(info.pid) and _is_sanic(info.pid):
